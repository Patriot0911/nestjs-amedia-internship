...
export class NewsService {
  ...
  @InjectRepository(NewsPostContentData)
  private readonly newsDataContentRepository: Repository<NewsPostContentData>
  ...
  async postNewsById(body: PostNewsDto) {
  ...
  async putNewsById(id: string, body: PutNewsDto): Promise<INewsPutResponse> {
    ...
    const modBaseNews: NewsEntity = {
      ...
      slug: body.slug,
      newsCategory: {
        ...baseNews.newsCategory,
        id: body.newsCategory.id
      }
    }
    const curContentNews = baseNews.newsContent.find(
      item => item.lang === body.lang
    )
    const bodyCurContent = body.translationList.find(
      item => item.lang === body.lang
    )
